...
const APP_VERSION = (window.HB_DATA && HB_DATA.version) || 'v0.27';
...
function bindHomeRecipeDelegation(){
  // Safety fallback only. The actual home cards are bound directly after every renderHome().
  document.addEventListener('click', event => {
    const target = event.target.closest('[data-home-recipe-id]');
    if(!target) return;
    const isHomeRecipe = target.id === 'dailyDrinkCard' || target.classList.contains('home-menu-item');
    if(!isHomeRecipe) return;
    event.preventDefault();
    openHomeRecipe(target.dataset.homeRecipeId);
  });
}

function openHomeRecipe(id){
  if(!id) return;
  openRecipe(id);
}
...
function renderHome(){
  ...
  if(daily){
    ...
    els.dailyDrinkCard.onclick = event => {
      event.preventDefault();
      openHomeRecipe(daily.id);
    };
  }
  ...
  els.todayMenuGrid.querySelectorAll('[data-home-recipe-id]').forEach(button => {
    button.onclick = event => {
      event.preventDefault();
      openHomeRecipe(button.dataset.homeRecipeId);
    };
  });
}
